...
import {
  ...
  weeksDelinquent,
  UnknownCustomer,
} from "../../../src/chapter10/content04/IntroduceSpecialCase.js";
...
describe("billingPlan", () => {
  ...
  it('should give customer billing plan when it is not "unknown" ', () => {
    expect(billingPlan(new Site(new Customer("Ava", "Annual")))).to.equal(
      "Annual"
    );
  });
});
...
describe("changeBillingPlan", () => {
  it('do nothing to customer billing plan is customer is "unknown" ', () => {
    expect(changeBillingPlan(new Site("unknown"))).to.eql(
      new UnknownCustomer()
    );
  });
